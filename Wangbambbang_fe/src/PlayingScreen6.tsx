...
type PlayingScreenNavigationProp = StackNavigationProp<
  ...
  'Playing6'
>;
type PlayingScreenRouterProp = RouteProp<RootStackParamList, 'Playing6'>;
...
const PlayingScreen6: React.FunctionComponent<Props> = ({
  navigation,
  route,
}) => {
  ...
  const sendPost = async () => {
    try {
      ...
      navigation.navigate('Score', {scores: [...scores, score]});
    } catch (error: any) {
      if (error.response) {
